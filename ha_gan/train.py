...
parser.add_argument('--exp-name', default='images_only_128', type=str,
...
def main():
    ...
    trainset = Volume_Dataset(data_dir='../data/preprocessed/images/', fold=args.fold, num_class=args.num_class)
    ...
    if args.continue_iter != 0:
        ckpt_path = 'ha_gan/checkpoint/'+args.exp_name+'/G_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
        G.load_state_dict(ckpt['model'])
        g_optimizer.load_state_dict(ckpt['optimizer'])
        ckpt_path = 'ha_gan/checkpoint/'+args.exp_name+'/D_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
        D.load_state_dict(ckpt['model'])
        d_optimizer.load_state_dict(ckpt['optimizer'])
        ckpt_path = 'ha_gan/checkpoint/'+args.exp_name+'/E_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
        E.load_state_dict(ckpt['model'])
        e_optimizer.load_state_dict(ckpt['optimizer'])
        ckpt_path = 'ha_gan/checkpoint/'+args.exp_name+'/Sub_E_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
    ...
    summary_writer = SummaryWriter("ha_gan/checkpoint/"+args.exp_name)
    
    # save configurations to a dictionary
    with open(os.path.join("ha_gan/checkpoint/"+args.exp_name, 'configs.json'), 'w') as f:
    ...
    for iteration in range(args.continue_iter, args.num_iter):
        ...
        if iteration > 30000 and (iteration+1)%500 == 0:
            torch.save({'model':G.state_dict(), 'optimizer':g_optimizer.state_dict()},'ha_gan/checkpoint/'+args.exp_name+'/G_iter'+str(iteration+1)+'.pth')
            torch.save({'model':D.state_dict(), 'optimizer':d_optimizer.state_dict()},'ha_gan/checkpoint/'+args.exp_name+'/D_iter'+str(iteration+1)+'.pth')
            torch.save({'model':E.state_dict(), 'optimizer':e_optimizer.state_dict()},'ha_gan/checkpoint/'+args.exp_name+'/E_iter'+str(iteration+1)+'.pth')
            torch.save({'model':Sub_E.state_dict(), 'optimizer':sub_e_optimizer.state_dict()},'ha_gan/checkpoint/'+args.exp_name+'/Sub_E_iter'+str(iteration+1)+'.pth')
